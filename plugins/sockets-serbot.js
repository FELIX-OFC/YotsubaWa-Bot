...
/* ---------------------------
   HANDLER (no activar cooldown aquí)
   --------------------------- */
let handler = async (m, { conn, args, usedPrefix, command, isOwner }) => {
  ...
  // NOTE: ya no guardamos Subs aquí — lo guardamos solo después de enviar QR o code
}
    ...
    async function connectionUpdate(update) {
      ...
      if (qr && !isCodeMode) {
        if (m?.chat) {
          ...
          // -> guardamos cooldown SOLO cuando mostramos el QR
          try {
            if (!global.db.data.users) global.db.data.users = {}
            if (!global.db.data.users[m.sender]) global.db.data.users[m.sender] = {}
            global.db.data.users[m.sender].Subs = Date.now()
          } catch (e) {
            console.error('No se pudo guardar Subs cooldown (QR):', e)
          }
        } else {
          return
        }
      ...
      if (qr && isCodeMode) {
        ...
        while (!secret && attempts < 3) {
          try {
            ...
            console.log('WS readyState:', sock?.ws?.readyState)
        ...
        if (secret) {
          ...
          // -> guardamos cooldown SOLO cuando mostramos el CODE
          try {
            if (!global.db.data.users) global.db.data.users = {}
            if (!global.db.data.users[m.sender]) global.db.data.users[m.sender] = {}
            global.db.data.users[m.sender].Subs = Date.now()
          } catch (e) {
            console.error('No se pudo guardar Subs cooldown (CODE):', e)
          }
