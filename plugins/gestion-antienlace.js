...
const handler = async (m, { conn, usedPrefix, command, args, isAdmin, isOwner }) => {
  ...
  if (['antienlace', 'antilink'].includes(command)) {
    ...
    if (args[0] === 'on' || args[0] === 'enable') {
      ...
      if (!isAntiLink) return conn.reply(m.chat, `☃️ La función *${command}* ya estaba *desactivada*.`, m, rcanal)
      isAntiLink = false
    } else {
      return conn.reply(
        m.chat,
...
export async function detectLink(conn, m) {
  ...
  if (linkRegex.test(m.text)) {
    // Responder primero
    await conn.sendMessage(m.chat, { text: '☃️ No envíes enlace si no quieres que te eliminé.' }, { quoted: m })
